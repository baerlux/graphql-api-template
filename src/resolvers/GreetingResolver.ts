import { Greeting, GreetingInput } from '../entity/Greeting'
...
export class GreetingResolver {
  ...
  async createGreeting(@Arg('msg') msg: string, @Arg('langId') langId: string) {
    const greeting = await Greeting.create({ msg, langId }).save()
  ...
  async updateGreeting(
    @Arg('id') id: number,
    @Arg('input', () => GreetingInput) input: GreetingInput
  ) {
  ...
  @Query(() => [Greeting])
  async allGreetings() {
    return await Greeting.find({ relations: ['lang'] })
  }
}
